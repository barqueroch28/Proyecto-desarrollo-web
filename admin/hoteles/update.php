<?php
    ...
    if(isset($_POST["id_hotel"]) && !empty($_POST["id_hotel"])){
        ...
        if(empty($nombre_err) && empty($ubicacion_err) && empty($descr_err) && empty($costo_err)){
            ...
            $sql = "UPDATE tours.hotel SET nombre=?, ubicacion=?, descripcion=?, costo=?, imagen=? WHERE id_hotel=?";
        ...
        // Check existence of id_hotel parameter before processing further
        if(isset($_GET["id_hotel"]) && !empty(trim($_GET["id_hotel"]))){
            // Get URL parameter
            $id_hotel =  trim($_GET["id_hotel"]);
            
            // Prepare a select statement
            $sql = "SELECT * FROM tours.hotel WHERE id_hotel = ?";
            if($stmt = mysqli_prepare($con, $sql)){
                // Bind variables to the prepared statement as parameters
                ...
                if(mysqli_stmt_execute($stmt)){
                    ...
                    if(mysqli_num_rows($result) == 1){
                        ...
                        // URL doesn't contain valid id_hotel. Redirect to error page
                        header("location: hoteles.php");
                        exit();
                    }
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <a href="hoteles.php" class="btn btn-secondary ml-2">Cancelar</a>
                    </form>
                </div>
            </div>        
        </div>
