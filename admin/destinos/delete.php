...
if (isset($_POST["id_destino"]) && !empty($_POST["id_destino"])) {
    ...
    $sql = "DELETE FROM tours.destino WHERE id_destino = ?";
    ...
    if ($stmt = mysqli_prepare($con, $sql)) {
        ...
        // Set parameters
        $param_id = trim($_POST["id_destino"]);

        // Attempt to execute the prepared statement
        if (mysqli_stmt_execute($stmt)) {
            $query = "ALTER TABLE tours.destino AUTO_INCREMENT = 1";
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        <div class="alert alert-danger">
                            <input hidden id="id_destino" name="id_destino" value="<?php echo trim($_GET["id_destino"]); ?>" />
                            <p>Está seguro que quiere eliminar este registro?</p>
                            <p>
                                <input type="submit" value="Si" class="btn btn-danger">
                                <a href="destinos.php" class="btn btn-secondary">No</a>
                            </p>
                        </div>
                    </form>
                </div>
